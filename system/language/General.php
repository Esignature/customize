...
class CI_General {
    ...
    <div id="contents" class="ul_crbt_list tab_container_div">
    	...
    	foreach($q->result() as $r){
			 $anchTrk = anchor(site_url('artist/profile/'.$r->artist_cid.'/'.$r->artist_slug.'/crbt/'.base64_encode($r->album_content_id.'/'.$r->content_id)), truncateStr($r->title,17,true), 'title="'.$r->title.'"');
			 $anchAlb= anchor(site_url('artist/profile/'.$r->artist_cid), truncateStr($r->artist_display,17,true), array('target'=>'_blank'));
			 $anchDownl = anchor(site_url('artist/profile/'.$r->artist_cid.'/'.$r->artist_slug.'/crbt/'.base64_encode($r->album_content_id.'/'.$r->content_id)), '<img src="images/download_icon.png" width="25" height="28" border="0" />', 'title="'.$r->title.'"');
        ...
        <ul>
          <li>';
       	...
       	foreach($q_latest->result() as $r_l){
			 $anchTrk_l = anchor(site_url('artist/profile/'.$r_l->artist_cid.'/'.$r_l->artist_slug.'/crbt/'.base64_encode($r_l->album_content_id.'/'.$r_l->content_id)), $r_l->title, 'title="'.$r_l->title.'"');
			 $anchAlb_l= anchor(site_url('artist/profile/'.$r_l->artist_cid), $r_l->artist_display, array('target'=>'_blank'));
			 $anchDownl = anchor(site_url('artist/profile/'.$r_l->artist_cid.'/'.$r_l->artist_slug.'/crbt/'.base64_encode($r_l->album_content_id.'/'.$r_l->content_id)), '<img src="images/download_icon.png" width="25" height="28" border="0" />', 'title="'.$r_l->title.'"');

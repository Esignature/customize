...
browser.initFiles = function() {
    ...
    $j('#files').unbind();
    $j('#files').scroll(function() {
        browser.hideDialog();
    });
    $j('.file').unbind();
    $j('.file').click(function(e) {
        _.unselect();
        browser.selectFile($j(this), e);
    });
    $j('.file').rightClick(function(e) {
        _.unselect();
        browser.menuFile($j(this), e);
    });
    $j('.file').dblclick(function() {
        _.unselect();
        browser.returnFile($j(this));
    });
    $j('.file').mouseup(function() {
        _.unselect();
    });
    $j('.file').mouseout(function() {
        _.unselect();
    });
    $j.each(this.shows, function(i, val) {
        var display = (_.kuki.get('show' + val) == 'off') ? 'none' : 'block';
        $j('#files .file div.' + val).css('display', display);
    });
    this.statusDir();
};
...
browser.showFiles = function(callBack, selected) {
    ...
    setTimeout(function() {
        var html = '';
        $j.each(browser.files, function(i, file) {
            var stamp = [];
            $j.each(file, function(key, val) {
        ...
        $j('#files').html('<div>' + html + '<div>');
        $j.each(browser.files, function(i, file) {
            var item = $j('#files .file').get(i);
            $j(item).data(file);
...
browser.selectFile = function(file, e) {
    if (e.ctrlKey || e.metaKey) {
        ...
        var files = $j('.file.selected').get();
        var size = 0;
        if (!files.length)
            this.statusDir();
        else {
            $j.each(files, function(i, cfile) {
                size += parseInt($j(cfile).data('size'));
            });
            size = this.humanSize(size);
            if (files.length > 1)
                $j('#fileinfo').html(files.length + ' ' + this.label("selected files") + ' (' + size + ')');
            else {
                var data = $j(files[0]).data();
                $j('#fileinfo').html(data.name + ' (' + this.humanSize(data.size) + ', ' + data.date + ')');
            }
        }
    } else {
        var data = file.data();
        $j('.file').removeClass('selected');
        file.addClass('selected');
        $j('#fileinfo').html(data.name + ' (' + this.humanSize(data.size) + ', ' + data.date + ')');
    }
};
...
browser.selectAll = function(e) {
    if ((!e.ctrlKey && !e.metaKey) || ((e.keyCode != 65) && (e.keyCode != 97)))
        return false;
    var files = $j('.file').get();
    if (files.length) {
        var size = 0;
        $j.each(files, function(i, file) {
            if (!$j(file).hasClass('selected'))
                $j(file).addClass('selected');
            size += parseInt($j(file).data('size'));
        });
        size = this.humanSize(size);
        $j('#fileinfo').html(files.length + ' ' + this.label("selected files") + ' (' + size + ')');
    }
...
browser.returnFile = function(file) {
    ...
    if (this.opener.CKEditor) {
        ...
        if (window.parent && window.parent.KCFinder) {
            var button = $j('#toolbar a[href="kcact:maximize"]');
...
browser.returnFiles = function(files) {
    if (this.opener.callBackMultiple && files.length) {
        var rfiles = [];
        $j.each(files, function(i, file) {
            rfiles[i] = browser.uploadURL + '/' + browser.dir + '/' + $j(file).data('name');
...
browser.returnThumbnails = function(files) {
    if (this.opener.callBackMultiple) {
        ...
        var j = 0;
        $j.each(files, function(i, file) {
            if ($j(file).data('thumb')) {
                rfiles[j] = browser.thumbsURL + '/' + browser.dir + '/' + $j(file).data('name');
...
browser.menuFile = function(file, e) {
    ...
    if (file.hasClass('selected') && files.length && (files.length > 1)) {
        ...
        $j('.menu a[href="kcact:pick"]').click(function() {
            browser.returnFiles(files);
            browser.hideDialog();
            return false;
        });

        $j('.menu a[href="kcact:pick_thumb"]').click(function() {
        ...
        $j('.menu a[href="kcact:download"]').click(function() {
            ...
            var pfiles = [];
            $j.each(files, function(i, cfile) {
                pfiles[i] = $j(cfile).data('name');
            });
            browser.post(browser.baseGetData('downloadSelected'), {dir:browser.dir, files:pfiles});
            return false;
        });

        $j('.menu a[href="kcact:clpbrdadd"]').click(function() {
            browser.hideDialog();
            var msg = '';
            $j.each(files, function(i, cfile) {
                var cdata = $j(cfile).data();
                var failed = false;
        ...
        $j('.menu a[href="kcact:rm"]').click(function() {
            if ($j(this).hasClass('denied')) return false;
            browser.hideDialog();
            var failed = 0;
            var dfiles = [];
            $j.each(files, function(i, cfile) {
        ...
        $j('.menu a[href="kcact:download"]').click(function() {
            ...
            '</form>';
            $j('#dialog').html(html);
            $j('#downloadForm input').get(0).value = browser.dir;
            $j('#downloadForm input').get(1).value = data.name;
        ...
        $j('.menu a[href="kcact:mv"]').click(function(e) {
    ...
    $j('.menu a[href="kcact:view"]').click(function() {
        ...
        var showImage = function(data) {
            ...
            var onImgLoad = function() {
                browser.lock = false;
                $j('#files .file').each(function() {
                    if ($j(this).data('name') == data.name)
                        browser.ssImage = this;
                });
                $j('#loading').css('display', 'none');
                $j('#dialog').html('<div class="slideshow"><img /></div>');
                $j('#dialog img').attr({
                    src: url,
                    title: data.name
                }).fadeIn('fast', function() {
                    var o_w = $j('#dialog').outerWidth();
                    var o_h = $j('#dialog').outerHeight();
                    var f_w = $j(window).width() - 30;
                    var f_h = $j(window).height() - 30;
                    if ((o_w > f_w) || (o_h > f_h)) {
                        if ((f_w / f_h) > (o_w / o_h))
                    ...
                    $j('#dialog').click(function(e) {
                        ...
                        if (browser.ssImage) {
                            browser.selectFile($j(browser.ssImage), e);
                        }
                    });
                    browser.showDialog();
                    var images = [];

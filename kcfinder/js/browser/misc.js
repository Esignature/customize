...
browser.drag = function(ev, dd) {
    ...
    if (top < 0) top = 0;
    if (left < 0) left = 0;
    if (top + $j(this).outerHeight() > $j(window).height())
        top = $j(window).height() - $j(this).outerHeight();
    if (left + $j(this).outerWidth() > $j(window).width())
        left = $j(window).width() - $j(this).outerWidth();
    $j(this).css({
        top: top,
        left: left
    });
...
browser.showDialog = function(e) {
    $j('#dialog').css({left: 0, top: 0});
    this.shadow();
    if ($j('#dialog div.box') && !$j('#dialog div.title').get(0)) {
        var html = $j('#dialog div.box').html();
        var title = $j('#dialog').data('title') ? $j('#dialog').data('title') : "";
        html = '<div class="title"><span class="close"></span>' + title + '</div>' + html;
        $j('#dialog div.box').html(html);
        $j('#dialog div.title span.close').mousedown(function() {
            $j(this).addClass('clicked');
        });
        $j('#dialog div.title span.close').mouseup(function() {
            $j(this).removeClass('clicked');
        });
        $j('#dialog div.title span.close').click(function() {
            browser.hideDialog();
            browser.hideAlert();
        });
    }
    $j('#dialog').drag(browser.drag, {handle: '#dialog div.title'});
    $j('#dialog').css('display', 'block');

    if (e) {
        var left = e.pageX - parseInt($j('#dialog').outerWidth() / 2);
        var top = e.pageY - parseInt($j('#dialog').outerHeight() / 2);
        if (left < 0) left = 0;
        if (top < 0) top = 0;
        if (($j('#dialog').outerWidth() + left) > $j(window).width())
            left = $j(window).width() - $j('#dialog').outerWidth();
        if (($j('#dialog').outerHeight() + top) > $j(window).height())
            top = $j(window).height() - $j('#dialog').outerHeight();
        $j('#dialog').css({
            left: left + 'px',
            top: top + 'px'
        });
        ...
        $j('#dialog').css({
            left: parseInt(($j(window).width() - $j('#dialog').outerWidth()) / 2) + 'px',
            top: parseInt(($j(window).height() - $j('#dialog').outerHeight()) / 2) + 'px'
        });
...
browser.hideDialog = function() {
    this.unshadow();
    if ($j('#clipboard').hasClass('selected'))
        $j('#clipboard').removeClass('selected');
    $j('#dialog').css('display', 'none');
    $j('div.folder > a > span.folder').removeClass('context');
    $j('#dialog').html('');
    $j('#dialog').data('title', null);
    $j('#dialog').unbind();
...
browser.showAlert = function(shadow) {
    $j('#alert').css({left: 0, top: 0});
    if (typeof shadow == 'undefined')
        shadow = true;
    if (shadow)
        this.shadow();
    var left = parseInt(($j(window).width() - $j('#alert').outerWidth()) / 2),
        top = parseInt(($j(window).height() - $j('#alert').outerHeight()) / 2);
    ...
    if ($j('#alert').outerHeight() > wheight) {
        $j('#alert div.message').css({
            height: wheight - $j('#alert div.title').outerHeight() - $j('#alert div.ok').outerHeight() - 20 + 'px'
        });
    }
    $j(document).unbind('keydown');
    $j(document).keydown(function(e) {
...
browser.hideAlert = function(shadow) {
    ...
    $j('#alert').html('');
    $j('#alert').data('title', null);
};

browser.alert = function(msg, shadow) {
    msg = msg.replace(/\r?\n/g, "<br />");
    var title = $j('#alert').data('title') ? $j('#alert').data('title') : browser.label("Attention");
    $j('#alert').html('<div class="title"><span class="close"></span>' + title + '</div><div class="message">' + msg + '</div><div class="ok"><button>' + browser.label("OK") + '</button></div>');
    $j('#alert div.ok button').click(function() {
        browser.hideAlert(shadow);
    });
    $j('#alert div.title span.close').mousedown(function() {
        $j(this).addClass('clicked');
    });
    $j('#alert div.title span.close').mouseup(function() {
        $j(this).removeClass('clicked');
    });
    $j('#alert div.title span.close').click(function() {
        browser.hideAlert(shadow);
    });
    $j('#alert').drag(browser.drag, {handle: "#alert div.title"});
    browser.showAlert(shadow);
};

browser.confirm = function(question, callBack) {
    $j('#dialog').data('title', browser.label("Question"));
    $j('#dialog').html('<div class="box"><div class="question">' + browser.label(question) + '<div class="buttons"><button>' + browser.label("Yes") + '</button> <button>' + browser.label("No") + '</button></div></div></div>');
    browser.showDialog();
    $j('#dialog div.buttons button').first().click(function() {
        if (callBack)
            callBack(function() {
                browser.hideDialog();
            });
    ...
    $j('#dialog div.buttons button').last().click(function() {
        browser.hideDialog();
    });
    $j('#dialog div.buttons button').get(1).focus();
};
...
browser.fileNameDialog = function(e, post, inputName, inputValue, url, labels, callBack, selectAll) {
    ...
    '</div></div></form>';
    $j('#dialog').html(html);
    $j('#dialog').data('title', this.label(labels.title));
    $j('#dialog input[name="' + inputName + '"]').attr('value', inputValue);
    $j('#dialog').unbind();
    $j('#dialog').click(function() {
        return false;
    });
    ...
    $j('#dialog').css('display', 'block');
    $j('#dialog input[type="submit"]').click(function() {
        return $j('#dialog form').submit();
    });
    var field = $j('#dialog input[type="text"]');
    var value = field.attr('value');
    if (!selectAll && /^(.+)\.[^\.]+$j/ .test(value)) {

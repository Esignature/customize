...
browser.initClipboard = function() {
    ...
    var size = 0;
    $j.each(this.clipboard, function(i, val) {
        size += parseInt(val.size);
    });
    size = this.humanSize(size);
    $j('#clipboard').html('<div title="' + this.label("Clipboard") + ' (' + this.clipboard.length + ' ' + this.label("files") + ', ' + size + ')" onclick="browser.openClipboard()"></div>');
    var resize = function() {
        $j('#clipboard').css({
            left: $j(window).width() - $j('#clipboard').outerWidth() + 'px',
            top: $j(window).height() - $j('#clipboard').outerHeight() + 'px'
        });
    };
    resize();
    $j('#clipboard').css('display', 'block');
    $j(window).unbind();
...
browser.openClipboard = function() {
    if (!this.clipboard || !this.clipboard.length) return;
    if ($j('.menu a[href="kcact:cpcbd"]').html()) {
    ...
    setTimeout(function() {
        ...
        $j('.menu a[href="kcact:rmcbd"]').click(function() {
            browser.hideDialog();
            browser.confirm(
        ...
        $j('.menu a[href="kcact:clrcbd"]').click(function() {
            browser.hideDialog();
            browser.clearClipboard();
            return false;
        });

        var left = $j(window).width() - $j('#dialog').outerWidth();
        var top = $j(window).height() - $j('#dialog').outerHeight() - $j('#clipboard').outerHeight();
        var lheight = top + _.outerTopSpace('#dialog');
        $j('.menu .list').css('max-height', lheight + 'px');
        var top = $j(window).height() - $j('#dialog').outerHeight() - $j('#clipboard').outerHeight();
        $j('#dialog').css({
            left: (left - 4) + 'px',
            top: top + 'px'

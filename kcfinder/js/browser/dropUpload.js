...
browser.initDropUpload = function() {
    ...
    filesDrop = function(e) {
        ...
        $j('#files').removeClass('drag');
        if (!$j('#folders span.current').first().parent().data('writable')) {
    ...
    folderDrop = function(e, dir) {
        ...
        for (var i = 0; i < e.dataTransfer.files.length; i++) {
            var file = e.dataTransfer.files[i];
            file.thisTargetDir = $j(dir).data('path');
            uploadQueue.push(file);
        }
        processUploadQueue();
        return false;
    ...
    folders.each(function() {
        ...
        dragOver = function(e) {
            $j(folder).children('span.folder').addClass('context');
            return folderDrag(e);
        },

        dragLeave = function(e) {
            $j(folder).children('span.folder').removeClass('context');
            return folderDrag(e);
        },
    ...
    function updateProgress(evt) {
        ...
        $j('#loading').html(browser.label("Uploading file {number} of {count}... {progress}", {

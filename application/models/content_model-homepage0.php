...
class Content_model extends CI_Model
{
	...
	public function getAlbum($limit=40, $offset=0, $filter=NULL)
	{
		...
		$s.= "M.image_path as image, ART.content_id AS artist_content_id, ART.title AS artist ";
	...
	{
		$s = "SELECT C.content_id, C.title, C.slug, C.ver1, A.itunes_url, A.release_date, M.image_path as image, ART.content_id AS artist_content_id, ART.title AS artist ";
		$s.= "FROM tbl_content C ";
		$s.= "INNER JOIN tbl_album A ON C.content_id = A.content_id ";
		$s.= "INNER JOIN tbl_artist_in_content AC ON C.content_id = AC.content_id "; 
  		$s.= "INNER JOIN tbl_content ART ON AC.artist_content_id = ART.content_id ";  
	...
	*/
	...
	public function getrelatedcontent($content_id,$mod_id,$del_flag=0,$limit=8,$tbl_name='tbl_video',$join=true){
	...
	public function getartistids($content_id,$mod_id=0,$del_flag=0){
	...
	public function getrelatedarticle($content_id,$type="Article",$mod_id=2,$related=true){
      ...
      <div class="title_wrapper">';
		...
		if($type=="Article"){
       		 $v.='<div class="articles_sponser"><img original-title="Nepalaya" class="sponsor_title" src="'.base_url().'images/sponser/nepalaya.jpg"></div>';
		}
    ...
    <div class="articles_contents">';
	 ...
	 $cls = ($i%2 == 0) ? 'left' : 'right';
		...
		$v.='<div class="article_album_'.$cls.'_wrapper">
				...
				{
					if(!empty($img_path))
					$link= '<img src="'.base_url().'uploads/145x85/'.$img_name.'" alt="'.$img_caption.'" width="70" >';
					else
				...
				/*
						if($img_name != ''){
							$link = '<img src="'.$img_path.'" border="0" width="70"  alt="'.$img_caption.'" title="'.$img_caption.'"/>';
						} else {
							$link = '<img src="'.base_url().'uploads/noimg.jpg'.'" border="0" width="70" height="53" alt="'.$img_caption.'" title="'.$img_caption.'"/>';
...
foreach($result as $r):
    ...
    $int_list.= '<span><em>'.ago($r->posted_date).'</em></span>';
    $int_list.= '</div> ';
    $int_list.= '</li>';
endforeach;

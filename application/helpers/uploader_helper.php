...
{
	function createImageThumb($image='', $new_width=190, $new_height=190, $folder='thumbs', $root='uploads', $passedFullPath=false,$header=false)
	{
				
		$imagesize=$passedFullPath ? getimagesize("{$image}") : getimagesize("./{$root}/{$image}");
		
		if(!$header && file_exists("./{$root}/{$folder}/{$image}")){
		    return false;
		}
		...
		$imagefile=$passedFullPath ? "{$image}" : "./{$root}/{$image}";
		...
		if ($imagesize[2] == "2")
		{
			...
			if($header){
				...
				$location="./{$root}/{$folder}/{$thename}";die(var_dump($imagesize));
...
{
    function crop($img='', $fr_fld='', $to_fld='thumbs/', $w=416, $h=231, $offx=0, $offy=0)
	{
		$ret = false;
		$upload_dir = './uploads/';
		$image = $upload_dir.$fr_fld.$img;
		$thumb = $upload_dir.$to_fld.$img;
		//echo $img.':'.$image.':'.$thumb."<br />";

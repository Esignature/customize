...
(function($) {
    ...
    $.widget('ui.spinner', {
        _init : function() {
            ...
            // switch off autocomplete in opera
        ...
        _mousewheel : function(e, delta) {
            var self = this;
            delta = ($.browser.opera ? -delta / Math.abs(delta) : delta); (delta > 0 ? self._up(e) : self._down(e));
            if(self.timeout) {
        ...
        _setValue : function(newVal) {
            ...
            this.element.val(this.options.currency ? $.ui.spinner.format.currency(newVal, this.options.currency) : $.ui.spinner.format.number(newVal, this._decimals, this.options.number_sep));
        },
        _animate : function(d) {
            if(this.element.hasClass('ui-spinner-list') && ((d == 'up' && this._getValue() <= this.options.max) || (d == 'down' && this._getValue() >= this.options.min))) {
                this.element.animate({
                    marginTop : '-' + this._getValue() * this.element.parent().height()
                }, {
                    duration : 'fast',
                    queue : false
                });
        ...
        _addItem : function(obj, fmt) {
            if(!this.element.is('input')) {
                ...
                var html = obj;
                // string or object set it to html first

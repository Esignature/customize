$(function(){
	$('form.form01').submit(function(){
		...
		elm = $('#intro');
		elm.val(tinyMCE.get('intro').getContent());
		if($.trim(elm.val()) == ''){ alert('Quiz intro filed cannot be blank!'); elm.focus(); return false; }
		elm = $('#full_content');
		elm.val(tinyMCE.get('full_content').getContent());
		if($.trim(elm.val()) == ''){ alert('Post participation field can not be blank!'); elm.focus(); return false; }
		
		$('ul.tab_link li:eq(1) a').trigger('click');
		elm = $('#active_from');
		if($.trim(elm.val()) == ''){ alert('Select quiz publish date from!'); elm.focus(); return false; }
		var date_frm_val = Date.parse($.trim(elm.val()));
		elm = $('#active_to');
		if($.trim(elm.val()) == ''){ alert('Select quiz publish date to!'); elm.focus(); return false; }

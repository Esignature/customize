...
<div id="tabs">
	...
	<!--  START TAB 1 -->
    <div>
 <?=form_open('apanel/news_search', array('class'=>'search_form', 'method'=>'post'))?>
        <input type="text" name="search" id="search" value="<?=$search?>" /> 
        <input type="submit" value="Search" />
    </form>   
<p class="description_icons">
    
    <span class="starred popular">&nbsp;</span> Featured News 
...
<?php } ?>
		...
		<?=anchor('apanel/news_form/'.$r->content_id, $r->title, array('class'=>'tooltip', 'title'=>'Click to Edit this news.'))?></td>
        ...
        <ul>
			<?php
				...
				if($search != ''){
					$s1 = 'active';
					$s2 = '';
					$menu = anchor('apanel/news', '<img src="'.$path.'img/search.png" width="24" height="24" alt="List" />Search Result', array('class'=>$s1));
				}
			    echo anchor('apanel/news', '<img src="'.$path.'img/001_20.png" width="24" height="24" alt="List" />View all news', array('class'=>$s2));
				echo $menu;
				echo anchor('apanel/news_form', '<img src="'.$path.'img/addpage.png" width="24" height="24" alt="Add" />Add News');
			?>
        </ul>
   </div>
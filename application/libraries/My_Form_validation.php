...
class MY_Form_validation extends CI_Form_validation {
    ...
    //validate if slug being saved is available or not.
    function uniqslug($slug, $param){
            
        $this->CI->form_validation->set_message('uniqslug', 'The slug is not available. Please re-generate or type in different slug manually!');    
        list($table, $key_id) = preg_split('/,/', $param);
        
        if($key_id == null || !isset($key_id)) $key_id = 0; 
                
        $this->CI->db->select("slug");
        $this->CI->db->from('tbl_slug');
        $this->CI->db->where(array("slug"=>$slug, "table_name"=>$table));
        if($key_id>0)
            $this->CI->db->where(array("key_id !="=>$key_id));
        
        $rows= $this->CI->db->get();
        
        return $rows->num_rows() ? false:true;
    }  
    
    /* similar to slug_available method.
     use it as:
     $this->form_validation->set_rules('email', 'E-mail', 'required|valid_email|unique[tablename.column]');
    */  
    ...
    */
    function set_error($msg)
    {
        ...
        return ($this->CI->lang->line($msg) == FALSE) ? $msg : $this->CI->lang->line($msg);
    }
    ...
    */
    ...
    function valid_url($str){
        $CI = & get_instance();
        $CI->load->language('register', CURRENT_LANGUAGE);
        
        $isValid = filter_var($str, FILTER_VALIDATE_URL);
        if(!$isValid){
            $this->set_message('valid_url', $CI->lang->line('lang_inv_url'));
            return false;
        }
        return true;
    }
    ...
    /**
     * Real URL : check to see if the website really exists
     * 
     * @param    string
     * @return    string
     */
    ...
    {
         $isValid = @fsockopen("$url", 80, $errno, $errstr, 30);
         if(!$isValid){
            $this->set_message('real_url', $CI->lang->line('lang_unreal_url'));
            return false;
        }
        return true;

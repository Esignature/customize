...
class Translator extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->langDirs = array( APPPATH . 'language', 'system/language' );
    ...
    */
	...
	function index() {
        ...
        if(isset($_POST['addSlaveLang']) && isset($_POST['langDir'])){
            $this->lang_saved = $this->add_language();
        }elseif(isset($_POST['delAxn']) && isset($_POST['langDir'])){
            if(strtolower($_POST['delAxn']) != 'english'){
                if($this->delete_language()){
                    deleteDir("application/language/".$_POST['delAxn']);
                    deleteDir("system/language/".$_POST['delAxn']);
                }
            }       
        }
		...
		if ( $this->_configured() ) {
			...
			if ( $this->validated && $this->input->post('ConfirmSaveLang') ) {
				if ( $this->_translation_saved( $result ) ) {
					...
					$this->data['page_content'] = 'apanel/translator/confirmLangSaved';
					$this->data['page_title'] = 'Changes Saved';
				} else {
					$this->data['page_content'] = 'apanel/translator/saveFailed';
					$this->data['page_title'] = 'Failure';
				}
			}
			elseif ( $this->validated &&  $this->input->post('SaveLang') ) {
				$this->data['page_content'] = 'apanel/translator/saveLang';
				$this->data['page_title'] = 'Confirm';
			}
			else {
				$this->data['page_content'] = 'apanel/translator/translateLang';
				$this->data['page_title'] = 'Translate ' . $this->langModule;
			}

		}
		
		$this->load->view('apanel/translator/container', $this->data);
		
	}
	
    // validation callback to check if language folder or record in db exists for the given language name
    function _lang_exists($lang){
        $this->load->model('language_model', 'lm');
        $exists = $this->lm->lang_exists($lang);
        if($exists){
            $this->form_validation->set_message('_lang_exists', 'The language or language folder already exists.');
            return false;
        }
        return true;   
    }
    ...
    function add_language(){
        ...
        if(isset($_POST['addSlaveLang'])){
            $this->load->model('language_model', 'lm');
            $this->load->library('form_validation');
            
            $this->form_validation->set_rules('language', 'Language', 'trim|required|max_length[30]|callback__lang_exists|xss_clean');
            $this->form_validation->set_rules('code', 'Language Code', 'trim|required|max_length[2]|xss_clean');  
            
            if($this->form_validation->run()){
                $qarr['language']   = $this->input->post('language');
                $qarr['code']       = $this->input->post('code');
                $qarr['status']     = 1;
                $qarr['sortorder']  = $this->lm->getSortOrder();
                if($this->lm->saveLang($qarr)){
                    $new_folder = APPPATH.'language/'. $qarr['language'];
                    $new_sys_folder = 'system/language/'. $qarr['language'];
                                            
                    @mkdir($new_folder);
                    @mkdir($new_sys_folder);
                    
                    foreach (glob(APPPATH.'language/english/*.php') as $filename) {
                        $f = fopen($new_folder.'/'.basename($filename), 'w+');
                        fclose($f);
                    }
                    
                    foreach (glob('system/language/english/*.php') as $filename) {
                        $f = fopen($new_sys_folder.'/'.basename($filename), 'w+');
                        fclose($f);
                    }
                    //now create folder and files in the system/language folder                              
                    return true;
                }
            }
            return false;
    ...
    function delete_language(){
        ...
        if(isset($_POST['delAxn'])){
            $language = $_POST['delAxn'];
            $this->load->model('language_model', 'lm');
            return $this->lm->deleteLanguageByName($language);
        }        
    }
	...
	function _configured() {
		...
		if ( ! $this->langDir ) {
			...
			$this->data['page_content'] = 'apanel/translator/selectLangDirectory';
			$this->data['page_title'] = "Select Language Directory";
		}		
		elseif ( ! $this->masterLang ) {
			$this->data['languages'] = $this->ListLanguages();
			$this->data['page_content'] = 'apanel/translator/selectMasterLang';
			$this->data['page_title'] = "Select Master Language";
		}		
		elseif ( ! $this->input->post('slaveLang') ) {
		    fl($this->data);
            $this->data['lang_saved'] = $this->lang_saved === true ? '<div class="success">Language created successfully.</div>' : '';    
			$this->data['languages'] = $this->_list_languages();
			$this->data['page_content'] = 'apanel/translator/selectSlaveLang';
		...
		elseif ( ! $this->langModule ) {		    
            ...
            fl($this->data);
			$this->data['page_content'] = 'apanel/translator/selectModule';

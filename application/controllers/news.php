...
class News extends CI_Controller {
	...
	*/
	function index()
	{
		$data['title'] =  'News || Cellroti - Artists, News, Views, Reviews and everything about Nepali Music';
		...
		$relatedc=$this->content_model->getrelatedarticle('',"Article",2,false);
		...
		// Pagination
        $config['base_url']            = base_url().'index.php/news/index/'.$list_tag.'/';
		...
		foreach($q->result() as $r):
			...
			if($cn == 0){
				...
				$d_intro.= '</div><div class="see_all">'.anchor('news/detail/'.$r->content_id, 'Read More').'</div></div>';
			} 
			else 
			{
				  ...
				  $d_list.= anchor('news/detail/'.$r->content_id, $img); 
				  $d_list.= '</div></div>';
				  ...
				  $d_list.= '     <div class="newslist_title">'.anchor('news/detail/'.$r->content_id, $r->title).'</div>';                	
		...
		$data['d_list'] = $d_list;
		// get the latest article for right display.
		$cond = array('module_id' => 2, 'status' => 1, 'del_flag' => 0);
		$this->db->join('tbl_image', 'tbl_image.content_id = tbl_content.content_id', 'left');
		$data['q_related_articles'] = $this->db->get_where('tbl_content', $cond, 4, 0);
		...
		$this->load->view(SITE_FOLDER.'news_list.php', $data);
	}
	...
	*/
	function detail()
	{
		...
		$data['title'] =  'News Detail || Cellroti - Artists, News, Views, Reviews and everything about Nepali Music';
...
<div class="inner_content">
	...
	'.$by.'
		...
		<div class="content">'.$image.$r->full_content.'</div><div class="clear"></div></div><div class="clear"></div>
                          ...
                          <div>
<div id="fb-root"></div><script src="http://connect.facebook.net/en_US/all.js#xfbml=1"></script><fb:like href="'.base_url().'index.php/news/detail/'.$id.'" send="true" width="481" show_faces="false" action="recommend" font=""></fb:like>
						  </div>
                          <div class="tags">'.$tags.'</div>
                          <div ><div>';
/*
GEt related news from the tags.
...
if($total > 0){
	...
	// final query for related news..
	$q = $this->db->query($s_related);
	if($q->num_rows() == 0){
		$ret.= '<div class="clear"></div></div>';
	} else 
	{
		$ret.= '<div class="relatednews">Related News</div>
               <div class="nextbtn">'.anchor('news/index', '<img src="'.base_url().'images/nxt_prev_btn.gif" border="0" />').'</div>
               <div class="clear"></div></div>';
	    $ret.= '<div class="images"><ul>';
		...
		foreach($q->result() as $row):
			...
			if(strstr($img_path, 'sites/')){
				...
				@list($w, $h) = getimagesize(str_replace('http://www.cellroti.com', $_SERVER['DOCUMENT_ROOT'], $img_path));					
				if(intval($w)>intval($h))
					$dim= ' width="100%" ';
				elseif(intval($w)<intval($h))
					$dim= ' height="100%" ';
				else
					$dim= ' width="130" height="85" ';

				
				$ret.= '<li><div style="width:130px;height:85px;text-align:center;border:1px dotted #CCCCCC;padding:1px">'.anchor('news/detail/'.$row->content_id, '<img src="'.base_url().'resizer.php?s='.urlencode($img_path).'&w=145&h=85" '.$dim.' border="0" />').'</div><p>';
			}else{
				if(!file_exists($img_path))
					$ret.= '<li>'.anchor('news/detail/'.$row->content_id, '<div style="color:#4775FF;font-weight:bold;line-height:7em;height:85px;border:1px dotted #CCCCCC;text-align:center">No Image</div>').'<p>';
				else
					$ret.= '<li><div style="width:130px;height:85px;text-align:center;border:1px dotted #CCCCCC;padding:1px">'.anchor('news/detail/'.$row->content_id, '<img src="'.base_url().'resizer.php?s='.urlencode($img_path).'&w=130&h=85" width="130" height="85" border="0" />').'</div><p>';
			}
			$ret.= anchor('news/detail/'.$row->content_id, $row->title).'</p></li>';
		endforeach;
		$ret.= '</ul></div>';
	}
}
...
if( $this->comment->count_all($id, true, 1) == 0){
	...
	foreach($q->result() as $r):
		...
		//$loop.= '<div class="left_pic">'.anchor('news/detail/'.$r->content_id, '<img src="'.$img_path.'" alt="'.$img_caption.'" width="105" height="62" border="0" />').'</div>';
		$loop.= '<div class="newslist_img">
				<div style="width:145px;height:85px;overflow:hidden;text-align:center;background:#fff;">
				'.anchor('news/detail/'.$r->content_id, $img).'</div>';
		$loop.= '</div>';
		$loop.= '<div class="newstitle_container">';
		$loop.= '     <div class="posted_date">'.($r->posted_date != '' ? date('Y-m-d h:i a', strtotime($r->posted_date)): '').'</div>';
		$loop.= '     <div class="newslist_title">'.anchor('news/detail/'.$r->content_id, $r->title).'</div>';                	
	...
	$this->load->library('pagination');
	$per_page = 5;
	$config['base_url']            = base_url().'index.php/news/detail/'.$id.'/';
...
$comments = '<div class="comments"><div><div class="relatednews">'.$block_title.'</div>
	    ...
	    $data['content_id'] = $id;
		$data['content'] = $ret.$comments;
		$data['q_related_articles'] = $this->content_model->getRelatedRecords(2, $id, 4);
		$data['q_interview']    = $this->content_model->getInterviews(); 
		$data['q_gallery_list'] = $this->content_model->getGallery(3);
		$data['q_current_poll'] = $this->content_model->get_list(7, false, 1, 1, 0); // 7 :: tbl_module :: Poll
		$data['q_pages']  = $this->content_model->getPages();
		
		$this->load->view(SITE_FOLDER.'news_detail.php', $data);
	}
